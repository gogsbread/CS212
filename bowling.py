...
In a perfect bowling game:

    >>> bowling([10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10])
    300

But in theory, the rules bowling are as follows:
...
"""
FRAMES = 10
# Main() : Compute the total score for a player's game of bowling.
def bowling(balls,debug=False):
    score = 0
    index = 0 # points to the start of next frame
    frame = 1
    while(frame <= FRAMES):
